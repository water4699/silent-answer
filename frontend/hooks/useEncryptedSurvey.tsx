...
export function useEncryptedSurvey() {
  ...
  const decryptTallies = useCallback(async () => {
    if (!contractAddress || !instance || !ethersSigner) {
      setMessage("Connect your wallet to decrypt results.");
      return;
    }
    ...
    try {
      // Request decryption access for anyone (public access)
      const contract = new ethers.Contract(contractAddress, contractInfo.abi as ethers.InterfaceAbi, ethersSigner);
      try {
        const tx = await contract.requestDecryptionAccess();
        await tx.wait();
        setMessage("Access granted. Preparing decryption signature...");
      } catch (error) {
        // If access is already granted, continue anyway
        console.log("[useEncryptedSurvey] Access may already be granted, continuing...", error);
      ...
      await refreshSurvey(); // Refresh to update authorized viewers list
    } catch (error) {
      console.error("[useEncryptedSurvey] decryptTallies error", error);
      setMessage("Unable to decrypt tallies. Please try again.");
    } finally {
      setIsDecrypting(false);
    }
  }, [contractAddress, encryptedTallies, ethersSigner, instance, storage, contractInfo.abi, refreshSurvey]);
  ...
  // Allow anyone to decrypt - authorization check removed for public access
  const isAuthorizedViewer = true; // Always true now since anyone can request access
